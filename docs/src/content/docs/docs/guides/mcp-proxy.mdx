...
When AllAgents generates a proxied client config, it now does so through the public `allagents mcp proxy <serverUrl>` helper command. The older `allagents mcp proxy-stdio` form still works as a compatibility alias for existing configs.
