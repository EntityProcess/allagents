...
## Choosing Install Scope and Clients

`allagents plugin install` asks where to save a new declaration and which
clients should receive it. The review step shows the exact config path,
declaration, and clients before anything changes.
...
`--scope` and `--client` bypass their respective choosers. `--yes` skips only
the final confirmation. JSON, CI, and non-TTY runs never prompt and retain the
command's deterministic defaults when a target flag is omitted.
...
You can add a specific skill from a GitHub repo without installing the full
plugin first:
...
Source-backed skill installs use the same scope, client, review, and
...
See the [CLI reference](/docs/reference/cli/#skill-add) for all source formats.
