...
Servers AllAgents adds are tracked in `.allagents/sync-state.json`; pre-existing user-managed servers in client MCP configs (`.mcp.json`, `.vscode/mcp.json`, `.github/mcp.json`, `.codex/config.toml`) are never touched.
...
| `servers.<name>.proxy` | Yes (per entry) | Additional clients for this server, or `*` for every current and future project MCP client |
