...
Structured help includes usage guidance, options, examples, output schemas, and
interaction requirements when available. `--help --json` and
`--json --help` are equivalent when they follow the command path.
...
project, user, and named-profile destinations interactively. The TUI supports
listing and inspecting declarations, adding HTTP or stdio servers,
reauthenticating HTTP servers, reconciling generated client configuration, and
removing declarations. Credential values remain hidden from summaries and
selection screens.
...
Add a new MCP server to the selected destination and immediately reconcile it.
