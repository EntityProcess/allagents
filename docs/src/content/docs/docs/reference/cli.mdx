...
Add a new MCP server to `workspace.yaml` and immediately sync it to all
configured clients. For HTTP servers, AllAgents connects first, completes OAuth
when required, and routes selected clients through its built-in MCP client.
...
Force a fresh OAuth login for a workspace-managed HTTP MCP server. Pass the
...
AllAgents clears the cached OAuth credentials for that server, opens a browser,
and verifies a new connection. If the browser is on another device, paste the
complete loopback callback URL into the waiting terminal prompt.
