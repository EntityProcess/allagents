...
allagents mcp auth <serverUrl> [--header KEY=VALUE...]
...
Authorize an OAuth-enabled HTTP MCP server before connecting it through a
headless or remote machine. Open the printed authorization URL in any browser.
When that browser redirects to an unreachable loopback address, copy the full
URL from its address bar and paste it into the prompt. AllAgents validates the
registered callback address and OAuth state before exchanging the code, then
caches the resulting credentials for `mcp proxy`.
...
Restart or reconnect the MCP client after authorization so `mcp proxy` can use
the cached token.
...
`auth` is the interactive login command; `proxy` remains the helper written to generated client configs.
