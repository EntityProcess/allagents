...
choosers. AllAgents then shows the config path, resulting declaration, and
clients; confirmation defaults to **No**. `--scope` and `--client` bypass only
their respective choosers. `--yes` skips only the final confirmation.
...
interactive target review as `plugin install`: scope, clients, exact config
path, prospective declaration, and a final confirmation that defaults to
**No**. Explicit `--scope` and `--client` values bypass those choosers;
`--yes` skips only confirmation. JSON, CI, and non-TTY runs remain prompt-free.
