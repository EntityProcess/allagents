...
**Claude Code only:**
- `tools` — Claude tool names (`Read`, `Grep`, `Glob`, `Bash`, `Edit`, `Write`)
- `model` — Claude model aliases (`sonnet`, `opus`, `haiku`, `inherit`)
- `disallowedTools`, `permissionMode`, `maxTurns`, `skills`, `mcpServers`, `hooks`, `memory`, `background`, `effort`, `isolation`, `initialPrompt`

**GitHub Copilot only:**
- `tools` — Copilot tool names (`read`, `edit`, `search/codebase`, `web/fetch`)
- `model` — Copilot model names (`Claude Opus 4.5`, `GPT-5 (copilot)`)
- `target`, `disable-model-invocation`, `user-invocable`, `mcp-servers`, `metadata`, `agents`, `argument-hint`, `handoffs`
...
| MCP servers | `.claude/mcp.json` | `.copilot/mcp-config.json` (project) · `~/.copilot/mcp-config.json` (user) |
| WORKSPACE-RULES | Injected in `CLAUDE.md` | Injected in `AGENTS.md` |

### Skills Are the Strongest Interop Point

Skills (reusable prompt files in `skills/` directories) are read natively by both Claude Code and Copilot. They use the same format and the same frontmatter (`name`, `description`). When writing shared plugins, prefer skills over client-specific commands or hooks.
