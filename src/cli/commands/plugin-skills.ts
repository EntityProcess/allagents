...
const updateCmd = command({
  ...
  args: {
    ...
    yes: flag({
      ...
      short: 'y',
      description:
        'Run without prompts (deleted-upstream skills are retained)',
    }),
  },
    ...
    try {
      ...
      if (interactive) {
        ...
        for (const unit of prepared.plan.units) {
          ...
          if (unit.blockedByOutOfScope) {
            ...
            p.log.warn(
              `${unitDisplayName(unit)} also backs deleted skills outside the selected scope. Local copies will be kept and this source will not be updated. Rerun with --scope all to review them together.`,
            );
            continue;
          }
          ...
          const heading =
            removedEntries.length > 0
              ? `The following installed plugin entries were deleted from ${unitDisplayName(unit)} upstream:`
              : `The following skills from ${unitDisplayName(unit)} appear to have been deleted upstream:`;
          const lines = unit.deleted.map(
            (skill) =>
              `  • ${skill.pluginName}:${skill.subpath} (${skill.scope})`,
          );
          p.log.warn(`${heading}\n${lines.join('\n')}`);
      ...
      if (isJsonMode()) {
        ...
        for (const local of prepared.inventory.skippedLocalSources) {
          console.log(`${chalk.dim('–')} Skipped local source ${local}`);
        }
        for (const unitResult of results) {
          const label = unitResult.source;
          switch (unitResult.status) {
            case 'updated':
              console.log(`${chalk.green('✓')} Updated ${label}`);
              break;
            case 'removed':
              console.log(`${chalk.green('✓')} Removed deleted skills and updated ${label}`);
              break;
            case 'retained':
              console.log(
            ...
            case 'failed':
              console.error(
                `${chalk.red('✗')} Failed ${label}${unitResult.error ? `: ${unitResult.error}` : ''}`,
              );
              break;
          }
        }
