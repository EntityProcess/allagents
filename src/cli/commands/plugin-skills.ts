  ...
  handler: async ({ scope }) => {
    try {
      ...
      if (error instanceof Error) {
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills list', error: error.message });
          process.exit(1);
        }
  ...
  handler: async ({ skill, scope, plugin }) => {
    try {
      ...
      if (matches.length === 0) {
        ...
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills remove', error });
          process.exit(1);
        }
      ...
      if (matches.length > 1) {
        if (!plugin) {
          ...
          const error = `'${skill}' exists in multiple plugins:\n${pluginList}\n\nUse --plugin to specify: allagents skills remove ${skill} --plugin <name>`;
          if (isJsonMode()) {
            jsonOutput({ success: false, command: 'skills remove', error });
            process.exit(1);
          }
      ...
      if (targetSkill.disabled) {
        ...
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills remove', error: msg });
          process.exit(1);
        }
        console.log(msg);
        return;
      ...
      if (!result.success) {
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills remove', error: result.error ?? 'Unknown error' });
          process.exit(1);
        }
      ...
      if (error instanceof Error) {
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills remove', error: error.message });
          process.exit(1);
        }
        console.error(`Error: ${error.message}`);
        process.exit(1);
    ...
    try {
      ...
      if (list) {
        if (skillArg) {
          ...
          if (isJsonMode()) {
            jsonOutput({ success: false, command: 'skills add', error });
            process.exit(1);
          }
          console.error(`Error: ${error}`);
          process.exit(1);
        ...
        if (!discovered.success) {
          if (isJsonMode()) {
            jsonOutput({ success: false, command: 'skills add', error: discovered.error });
            process.exit(1);
          }
      ...
      if (all) {
        ...
        if (!installResult.success) {
          if (isJsonMode()) {
            jsonOutput({ success: false, command: 'skills add', error: installResult.error });
            process.exit(1);
          }
      ...
      if (matches.length === 0) {
        if (from) {
          ...
          if (!installFromResult.success) {
            if (isJsonMode()) {
              jsonOutput({ success: false, command: 'skills add', error: installFromResult.error });
              process.exit(1);
            }
      ...
      if (matches.length > 1) {
        if (!plugin) {
          const pluginList = matches.map((m) => `  - ${m.pluginName} (${m.pluginSource})`).join('\n');
          const error = `'${skill}' exists in multiple plugins:\n${pluginList}\n\nUse --plugin to specify: allagents skills add ${skill} --plugin <name>`;
          if (isJsonMode()) {
            jsonOutput({ success: false, command: 'skills add', error });
            process.exit(1);
          }
      ...
      if (!targetSkill.disabled) {
        ...
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills add', error: msg });
          process.exit(1);
        }
        console.log(msg);
        return;
      ...
      if (!result.success) {
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills add', error: result.error ?? 'Unknown error' });
          process.exit(1);
        }
        console.error(`Error: ${result.error}`);
        process.exit(1);
      ...
      if (error instanceof Error) {
        if (isJsonMode()) {
          jsonOutput({ success: false, command: 'skills add', error: error.message });
          process.exit(1);
        }
        console.error(`Error: ${error.message}`);
        process.exit(1);
